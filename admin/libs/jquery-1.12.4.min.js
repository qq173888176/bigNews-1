...
//每一个ajax请求都要设置令牌,不然人家以为你没有登录,那就不给你数据
//每一个ajax请求都要设置令牌,那就很烦人了,所以我们可以全局设置

//使用$.ajaxSetup()方法来配置全局的信息
//把$.ajaxSetup()方法写在jQuery文件中,
//是因为不管哪个页面使用jQuery发送ajax请求,都要先导入这个jQuery文件,那下面这个方法就执行了.
$.ajaxSetup({
  ...
  error(xhr, status, error) {
    ...
    //如果请求错误的信息是Forbidden ,说明没有带令牌去发送的请求,所以要打回登录页面.
    if (error == "Forbidden") {
      alert("请登录!");
